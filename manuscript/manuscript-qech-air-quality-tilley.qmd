...
custom_labeller <- function(labels) {
  lapply(labels, function(x) bquote(.(x)))
}
...
percent_exposure <- dat_air_quality_sum_who_2021 |> 
  ...
  mutate(indicator = case_when(
    indicator == "pm10" ~ "PM[10]",
    indicator == "pm2.5" ~ "PM[2.5]"
  )) |> 
  ungroup()
...
fig05 <- ggplot(data = percent_exposure,
  ...
  facet_grid(location~indicator, switch = "y", 
             labeller = labeller(location = custom_labeller, indicator = label_parsed)) +
...
df <- data.frame(
  x = rep(1:3, each = 10),
  y = rnorm(30),
  group = rep(c("A[1]", "B[2]", "C[3]"), each = 10),
  category = rep(c("Cat 1", "Cat 2"), 15)
)
...
# Create a ggplot with subscript labels in facets
p <- ggplot(df, aes(x, y)) +
  geom_point() +
  facet_grid(category ~ group, labeller = labeller(category = custom_labeller, group = label_parsed))
