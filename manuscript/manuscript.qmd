...
when just breathing is, as the quote used in the title of this article,
'unbearable.'
...
in diameter in mg/m^3^. A list of the specific hardware and software
...
the manuscript \[TODO: manuscript software citation\].
...
area and the open burning that occurs there. The residents at the
Guardian Shelter however are mostly cooking within contained clay stoves
that have been designed to improve combustion of the coal and wood that
is burned within them. The fewer number of PM~10~ peaks is a testament
to this intervention, though the frequency of PM~2.5~ peaks is still
beyond acceptable.
...
2.  Air quality monitoring code is written in Python using the [aqi
    ...
    concentration). The code is stored on the [Open Data Durban GitHub
    account](https://github.com/opendatadurban/hospital_stations) and is
    free for all to access. To be able to collect data, the device needs
    to be connected to a reliable wifi network, the SSID and password
    need to be coded onto the device during step 2 in the building
    method.
