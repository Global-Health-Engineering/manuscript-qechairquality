...
hospital campus \citep{kalina2020this}. However, the vast majority of
the deposited waste is not incinerated, as a new incinerator provided by
the Ministry of Health in late 2019, is only able to handle a small
percentage of the hospital's waste. Furthermore, the incinerator is
frequently inoperable, under repair, or without power,. Rather, the
...
refuse. Furthermore, caregivers who reside separately on hospital
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Figure 1: Open burning at QECH, with the old incinerator building in the background (Authors).}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(}\StringTok{"manuscript/img/image{-}replace{-}tree.png"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{img/image-replace-tree.png}
...
\item
  ...
  cerebral malaria, for more than 30 years. Its research infrastructure
  includes an administrative team, an inpatient research ward, an MRI
  centre, and an outpatient research clinic in Ndirande township and in
  other districts outside of Blantyre.
\item
...
\item
  ...
  cooking pavilion and toilet/shower block are maintained by a local
  NGO. Though simple, the facility is secure and offers one of the only
  free/accessible toilet facilities for visitors on the QECH campus.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# map}
\CommentTok{\# Figure 2: Sensor locations within QECH}
\end{Highlighting}
\end{Shaded}
...
software components can be found in Appendix TODO.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Table 1. Particulate matter targets from @worldhealthorganization2021who}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(}\StringTok{"manuscript/img/image{-}replace{-}table.png"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{img/image-replace-table.png}
...
presented in Appendix XA1 (TODO). However, the number of measured values
that exceeded the hazardous limit for both parameters are summarised in
Table 2.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Table 2. Peaks for both PM2.5 and PM10 are the number of data points above the WHO interim target 1 (annual) which is the least stringent;  the number of observations recorded (count) is provided for reference. }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Figure 3. Average 24{-}hour PM10 and PM2.5 at 8 monitoring stations over 2 months}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Figure 4. PM10:PM2.5 values for each location.  Each line shows the minimum and maximum ratio for the day based on ratios calculated hourly.  }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Figure 5. Percentage of measured values categorised according to WHO 2021 targets}
\end{Highlighting}
\end{Shaded}
